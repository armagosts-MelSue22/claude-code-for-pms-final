...
### Added 10 Sep 2026 (later) — after reading `data/callout-history.csv`
This file was already in the repo and materially changes several conclusions above. Weekly rows per responder, 29 Jun – 31 Aug, same 16 responders throughout. Ship week (10–16 Aug) excluded from before/after averages.

- **It's a redistribution, not a slowdown.** Total offers sent barely moved (~172/wk before → ~162 after). But four responders collapsed — Farlight −89% (0% acceptance since the release), The Undertow −83%, Vesper −81%, Meteor Mite −79% — while ten others gained +19% to +49%. Acceptance rate had sat in a 2.9-point band for six straight weeks (75.0–77.9%), fell to **54.2%** the ship week, and has recovered only to 72.7%.
- **Kip's pair is confirmed numerically:** Meteor Mite 11→1 offers/wk while The Gale 13→21, same handler, same city, same weeks. Seasonality cannot produce divergence inside a matched pair. Caveat: the file starts in June, so there is no year-over-year comparison and Priya's seasonal read can't be tested directly — only out-argued by the allocation evidence.
- **The ticket queue is a poor instrument here — do not triage off it.** Only 7 of 17 quiet-stretch tickets are corroborated (The Undertow, Farlight, Cpl. Ashgrove −27%, Halfmoon −24%). **Ten are contradicted** — Nightwell (+31%), Ironvale (+38%), Sgt. Falkirk (+47%), Stormwrack (+36%), Cindermark (+30%), The Drift (+24%), The Longcast (+21%) all reported silence while receiving *more* offers.
- **Unresolved, and the first question for Marcus:** either `pings_sent`/`pings_taken` don't mean what the column names imply (nothing documents them — if `sent` counts dispatch rather than delivery, some contradictions become a notification-delivery bug instead), or a large group of experienced handlers is wrong about their own responders. Resolve this *before* quoting any of these numbers or acting on the routing hypothesis.
- **The two worst-hit responders filed nothing.** Vesper (Dot's) and Meteor Mite (Kip's) appear in zero tickets and surfaced only because Sofia happened to interview their handlers. Support systematically under-reports the actual failures and over-reports where nothing measurable changed.
...
### Added 17 Sep 2026 — after walking the routing code file-by-file and quantifying the recovery path
- **Marcus's 14 Aug question is answered: it wasn't targeted.** `config.py`/`routing.py` apply one formula to every responder — no branch or flag treats habitual decliners differently. Confirmed against the data too: Vesper had the second-highest pre-4.2 acceptance rate on the roster (81.9%, not a decliner by any measure) and still lost 81% of her offers, same as the genuinely below-average Meteor Mite (68.7% pre-4.2). A reliable responder getting hit this hard is evidence the reweighting is a blanket effect, not a deliberate (if undocumented) decision aimed at chronic decliners.
- **Quantified how hard the climb back actually is.** From the score floor, it takes **7 consecutive accepted offers** to reach neutral again (recovery is +0.08/accept against a 0.5-wide gap), and because one miss costs more (−0.12) than one win gains, a single slip anywhere in that climb pushes the real number past 7 — there's no cushion.
- **That climb is not realistic to wait out.** Using the roster-wide accept rate (~75%) as a generous floor, the odds of 7 clean accepts in a row are ~13%. Using what actually happened to the four collapsed responders in the three weeks after ship (3 accepts out of 25 offers combined, ~12%), the odds drop to roughly **1 in 2.8 million**. Small sample, but it lines up with the fact that none of the four had recovered by 31 Aug. This is the strongest argument yet for building the decay fix rather than assuming September recovers it on its own.
- **No real commit history exists in this repo** — it's a single flattened "Initial commit" from 8 Sep, so authorship of the 4.2 `config.py` edit (the weight rebalance + timeout cut) can't be verified from git. Still an open, direct question for Wen or Marcus, sharpened by the PTO gap: Wen was out 14–24 Aug and missed Marcus's original question in the Slack thread.
- **Two more visuals built this session**, both in `00-rook/analysis/`: `4.2-routing-before-after.html` (Director-facing — weight/timeout comparison plus the suppression-loop diagram) and `quiet-phone-customer-story.html` (customer-facing — plain-language story plus the 7-step recovery-climb visual; no internal numbers, file paths, or responder names, safe to share externally).

### Added 23 Sep 2026 — after building Helen's requested one-pager and a clickable prototype
- **Helen's ask (`05-super-speed/director-request.txt`):** before anyone touches the code, she wants to see the proposed fix from the point of view of the person it happens to — not a setting — something a handler like Kip would notice and a responder who's gone quiet would feel differently about. A one-pager, ideally something clickable.
- **Built both.** The one-pager landed as `00-rook/analysis/recovery-path-brief.pdf` (mirrored as text in `05-super-speed/brief.md`), restructured into 3 succinct sections: the story (Wen's 2019 `history.py` TODO, told through Meteor Mite and Kip), open questions before coding, and where the code needs to change. The clickable version is `05-super-speed/prototype.html` — a single-file mock of the handler console's Responders screen with a Now / With-recovery-path toggle.
- **The brief's open-questions list is the real gate, not the two numbers Wen needs to size.** Even with a decay value chosen, nobody would be able to tell the fix is working (no visibility for Meteor Mite or Kip), "unoffered" isn't yet defined precisely enough to distinguish suppression from a genuine coverage gap, there's no backfill decision for responders already parked at the floor, no success bar, and no monitoring/rollback plan — 4.2 also shipped clean with no pages and still caused three weeks of undetected harm.
- **Where the fix actually lands in code:** `history.py` is the core of the change and needs a per-responder last-updated timestamp (doesn't exist today) plus decay logic, most likely computed lazily inside `recent_acceptance()`; `config.py` needs two new tunables alongside `ACCEPTANCE_CREDIT`/`DECLINE_PENALTY`; `offer.py` and `routing.py` shouldn't need changes; `availability.py` shouldn't either, but any shape change there still has to go through Supply first per that file's own note.
- **Building the prototype's volume + time simulator surfaced a real nuance worth keeping:** recovery isn't instant even once the fix ships — modeled at ~6 weeks to ease back to neutral, matching the brief's own diagram — and at low incident volume a responder can still look "at risk" for a while after the fix is live, because easing the score back doesn't help if there aren't enough incidents for it to turn into actual offers. Don't oversell the fix as an immediate cure for the quiet-phone complaints; September's lower volume could still mask it working.

### Added 24 Sep 2026 — after building a reusable brief-review skill
- **Closed one of the 23 Sep gate items:** `brief.md` now has an explicit "What 'worked' looks like" section (draft) — the four collapsed responders returning to their pre-4.2 offer-volume band within ~6 weeks, flagged if no movement by week 4. Still needs Wen's sign-off on the 6-week assumption and Sofia/Nadia's read on the cohort, but "no success bar" is no longer a blank on this brief.
- **Built a reusable `/review-checklist` skill** (`.claude/skills/review-checklist/`), triggered by phrases like "review this brief" or "grade this brief." It checks any brief against six fixed criteria — owner, definition of success, scope consistency (start vs. end), problem-before-fix ordering, named audience, and stated assumptions/gaps — and scores it as a composite percentage with a tiered reaction (Celebration >90%, Mediocre 60–90%, Failed <60%). `brief.md` currently passes clean at 6/6 (100%).
- Each run writes `<brief-filename>-review.html` next to the reviewed brief with the full scorecard — worth reusing on future brief drafts (Antje's and Harsha's brief-review.html in `05-super-speed/` were built as test cases for the skill itself, not part of the Dispatch investigation).
...
- **The pattern that looks compelling but mostly isn't** (superseded by the callout data — see the second 10 Sep block above): five responders appear to report a long silence and then a vanished offer — Nightwell (T-004→T-011), The Undertow (T-005/T-013→T-019), Ironvale (T-008/T-017→T-025), Cindermark (T-020), The Drift (T-022→T-023). **Only The Undertow's silence is real.** The other four received 24–38% *more* offers after 4.2; their timeout half stands, their silence half does not. Don't rebuild an argument on this sequence without checking the CSV first.
